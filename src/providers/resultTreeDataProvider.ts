...
export class ResultTreeDataProvider implements vscode.TreeDataProvider<ResultTreeItem> {
    ...
    private _results: QueryResult | undefined;
    ...
    // 更新查询结果
    updateResults(results: QueryResult): void {
    ...
    // 获取子项
    async getChildren(element?: ResultTreeItem): Promise<ResultTreeItem[]> {
        if (!this._results) {
        ...
        if (!element) {
            ...
            if (this._totalPages > 1) {
                const paginationItem = new ResultTreeItem(
                    `Page ${this._currentPage + 1} of ${this._totalPages}`,
                ...
                // 添加分页导航命令
                const prevPageItem = new ResultTreeItem(
                    'Previous Page',
                    vscode.TreeItemCollapsibleState.None
                );
                ...
                const nextPageItem = new ResultTreeItem(
                    'Next Page',
                    vscode.TreeItemCollapsibleState.None
                );
            ...
            // 添加导出命令
            const exportItem = new ResultTreeItem(
                'Export to CSV',
                vscode.TreeItemCollapsibleState.None
            );
            exportItem.command = {
            ...
            // 添加结果信息
            const rowCount = isRowDataPacketArray(this._results.rows) ? this._results.rows.length : 0;
            const resultInfoItem = new ResultTreeItem(
            ...
            // 添加表头
            const headerItem = new ResultTreeItem(
                'Columns',
            ...
            // 添加行数据根节点
            const rowsItem = new ResultTreeItem(
                'Rows',
                vscode.TreeItemCollapsibleState.Expanded
            );
        ...
        if (element.contextValue === 'rows') {
            if (!isRowDataPacketArray(this._results.rows)) {
            ...
            return pageRows.map((row: RowDataPacket, rowIndex: number) => {
                const actualRowIndex = startIndex + rowIndex;
                const rowItem = new ResultTreeItem(
        ...
        if (element.contextValue === 'row' && element.id) {
            ...
            return Object.entries(row).map(([key, value]: [string, any], index: number) => {
                const fieldName = this._results?.fields[index]?.name || key;
                const cellItem = new ResultTreeItem(
                    fieldName,
                    vscode.TreeItemCollapsibleState.None,
                    value
                );
    ...
    // 导出CSV方法
    exportToCsv(): string {
        if (!this._results || !this._results.rows || !isRowDataPacketArray(this._results.rows) || this._results.rows.length === 0) {
            return '';
        }
        
        const headers = this._results.fields.map(f => f.name).join(',');
        const rows = this._results.rows.map((row: RowDataPacket) => 
            Object.values(row).map(value => this.formatValueForCsv(value)).join(',')
        );
